...
> APEX does not assume Ollama/Nomic embeddings. On this install the preferred local semantic stack is:
>
> - Embedder: `http://127.0.0.1:8090/v1/embeddings`, model `intfloat/e5-base-v2`, 768 dimensions
> - Reranker: `http://127.0.0.1:8091/v1/rerank`, model `jinaai/jina-reranker-v1-tiny-en`
>
> See `mcp/local-semantic.json` for the canonical local service description.
...
For opencode, merge the `grepai` MCP entry into `~/.config/opencode/opencode.json` under `mcp`.
...
Or manually add the same server entry to your MCP settings.
